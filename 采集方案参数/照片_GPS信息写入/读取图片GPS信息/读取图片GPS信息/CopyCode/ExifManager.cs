...
namespace 读取图片GPS信息.CopyCode
{
    ...
    public class ExifManager : IDisposable
    {
        ...
        public Rational GPSLatitude
        {
            get
            {
                return this.GetPropertyRational((int)TagNames.GpsLatitude);
            }
            set
            {
        ...
        // Is recommended to use typed methods (like etc.) instead, when possible.
        // 
        // [altair] 10.30.2015 Created
        // 
        public void SetPropertyRational(Int32 PID,Rational Value)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                Value.Numerator >>= 8;
                Data[i + 4] = (byte)(Value.Denominator & 0xFF);
                Value.Denominator >>= 8;
            }
            SetProperty(PID,Data,ExifDataTypes.UnsignedRational);
        }
        public void SetLongitudeLatiProperty(Int32 PID, double Degree)
        {
